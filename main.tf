...
# EC2 deployment --------------------------------------------------
# Get ECR credentials for Docker
...
# Create an ECR repository to push to
...
resource "docker_image" "taskoverflow" { 
    ...
    build { 
        context = "." #Look in current directory for "Dockerfile"
    } 
} 
...
# Fetch the private subnets within the VPC
...
# Create the ECS cluster to run containers
resource "aws_ecs_cluster" "taskoverflow" { 
    name = "taskoverflow" 
}

# Create a task definition describing the container configuration for ECS
